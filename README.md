...
- `GROUP_MAX_DURATION_SEC` (default: `1800`): the maximum time span around the selected message for surrounding context and group continuation.
- `GROUP_MAX_INTERPOSTS` (default: `6`): the maximum number of interposts allowed while following the selected author's forward burst.
- `CTX_PREPEND_BEFORE` (default: `12`): the number of nearby messages from any author to include before the selected message.
- `CTX_APPEND_AFTER` (default: `20`): the number of nearby messages from any author to include after the selected message.
...
- `CTX_INCLUDE_INTERPOSTS` (default: `true`): whether the selected author's forward burst may include interposts after the selected message.
